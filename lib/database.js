...
export function addGroupMessage(message) {
  db.run(
    ...
    [message.id, message.sessionId || 'group-legacy', message.assistantId || 'default', message.sender, message.role, message.content || '', message.thinking || '', JSON.stringify(message.images || []), message.avatar || '🤖', message.color || '#c97b5a', message.favorite ? 1 : 0, message.parentMsgId || null]
  );
  saveDatabase();
}
