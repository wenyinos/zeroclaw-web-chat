...
export function setupWsProxy(server) {
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    try {
      ...
      // 群聊并发：客户端可为每个助手指定独立的网关侧会话标识（不影响本地鉴权）
      gwSessionId = (normalizedSearchParams.get('gw_session') || '').trim();
    ...
    if (pathname === '/ws/chat') {
      ...
      wss.handleUpgrade(request, socket, head, (clientWs) => {
        ...
        // 网关会话标识：优先用客户端指定的 gw_session（白名单校验，防注入），
        // 不合法或缺省时回退 auth_session（原行为）
        const gatewaySessionId = /^[A-Za-z0-9_-]{1,80}$/.test(gwSessionId) ? gwSessionId : authSessionId;
        const targetUrl = USE_PICOCLAW
          ? `${gatewayWsUrl}${gatewayWsPath}?session_id=${encodeURIComponent(gatewaySessionId)}`
          : `${gatewayWsUrl}${gatewayWsPath}${gatewayQuery.toString() ? `?${gatewayQuery.toString()}` : ''}`;
        ...
        // picoclaw 流式回复缓冲：message.create 建消息、message.update 原地累积，
        // 收到完成信号（带 usage/context_usage 的 update）或静默超时后一次性下发完整内容。
        // 原因：picoclaw 流式输出，若逐条透传，前端会把每个增量当成独立短消息渲染，
        // 且 message.update 在前端没有对应分支会被静默丢弃，最终只显示 create 的首片段。
        const picoReplies = new Map(); // message_id -> { content, contextUsage, lastSeen }
        const flushPicoReply = (messageId) => {
          const entry = picoReplies.get(messageId);
          if (!entry) return;
          picoReplies.delete(messageId);
          clientWs.send(JSON.stringify({
            type: 'message',
            content: entry.content,
            context_usage: entry.contextUsage
          }));
        };
        const flushTimer = setInterval(() => {
          const now = Date.now();
          for (const [messageId, entry] of picoReplies) {
            if (now - entry.lastSeen > 2500) {
              flushPicoReply(messageId);
            }
          }
        }, 1000);
        flushTimer.unref();
          ...
          if (clientWs.readyState === WebSocket.OPEN) {
            ...
            if (USE_PICOCLAW) {
              try {
                ...
                // picoclaw 流式: message.create 建消息、message.update 原地更新
                if (msg.type === 'message.create' || msg.type === 'message.update') {
                  const p = msg.payload || {};
                  const messageId = p.message_id;
                  const isThought =
                    (typeof p.kind === 'string' && p.kind.toLowerCase() === 'thought') ||
                    p.thought === true;
                  if (isThought) {
                    // 思考内容仍即时转发，前端走 thinking 面板
                    clientWs.send(JSON.stringify({ type: 'thinking', content: p.content }));
                    return;
                  }
                  if (messageId && typeof p.content === 'string') {
                    if (msg.type === 'message.create') {
                      // 新回复开始，先冲刷残留的旧缓冲（防止上一条未收到完成信号）
                      for (const [id] of picoReplies) flushPicoReply(id);
                    }
                    picoReplies.set(messageId, {
                      content: p.content,
                      contextUsage: p.context_usage,
                      lastSeen: Date.now()
                    });
                    // 带 usage/context_usage 的 update 是回复完成的最后一条，立即下发
                    if (msg.type === 'message.update' &&
                        (p.usage !== undefined || p.context_usage !== undefined)) {
                      flushPicoReply(messageId);
                    }
                  }
                  return;
