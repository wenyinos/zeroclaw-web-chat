...
- **Markdown 渲染**：标题、有序/无序/嵌套列表、任务列表、引用、表格（含列对齐）、分隔线、代码块（带语言标签，保留缩进换行、长行横向滚动）、行内代码、粗体/斜体/删除线、链接
- 聊天记录自动保存到 SQLite 数据库（原子写入：先写临时文件再原子替换，崩溃不会损坏数据库）
- **会话管理**：新建、恢复、导出 Markdown、删除（私聊与群聊各自独立）
- **会话时间线**：会话记录按「今天／昨天／本周更早／更早」分组，左侧列出每条会话的首条用户消息摘要、消息数与更新时间，右侧按角色与时间展示完整对话（同样走 Markdown 渲染）
...
- **助手设置**：表单化编辑名称、头像、颜色、提示词与触发词（必填与重名校验，触发词自动补 `@` 并去重），各助手按自己的人设回复
...
- 用户名和助手名称自定义
- **主题**：三套配色（暖色 / 粉色 / 浅蓝），每套各有亮色与暗色；顶部按钮在当前配色内切换明暗，设置页下拉可直选全部六种。主题按设备保存在本机，服务端设置里的值只作为新设备的初始值
- 浏览器通知开关
...
- `PUT /api/assistants/:id` — 更新助手（名称与提示词不可置空）
- `DELETE /api/assistants/:id` — 删除助手（默认助手不可删除）

### 会话管理
- `GET /api/sessions` — 获取会话列表（含 `createdAt` 与 `preview`：该会话首条用户消息摘要）
